...
! number of true grid points on each processor
      ...
      real*4 :: v11D(nx*np1), v11D_X(nx*np1)
      real*8 :: mdot(nx*np1), mdot_X(nx*np1) 
...
!      dx3 = (x3max-x3min)/(nz*np3)
      ...
      do l = 0, np1-1
        ...
        read(42) (((v1(i,j,k), i=1,nx), j=1,ny), k=1,nz)
        ...
        v1g(nx*l+1:nx*(l+1),ny*m+1:ny*(m+1),nz*n+1:nz*(n+1)) &
        = v1g(nx*l+1:nx*(l+1),ny*m+1:ny*(m+1),nz*n+1:nz*(n+1)) &
        + v1(1:nx,1:ny,1:nz)
      ...
      v1g = v1g/((iincr)*20-max(0,(iincr-1))*20+1)


! 1D profiles

      k = 1
      write(onefile,"(a3,i3.3,a11)") 'one', iincr, '_C10_cf.dat'
      ...
      do i = 1, np1*nx
        ...
        do j = 1, np2*ny
          ...
          v11D(i) = v11D(i) + v1g(i,j,k)*dvol
          mdot(i) = mdot(i) - 4.*pi*dvol*x1b(i)*dg(i,j,k)*x1b(i)*v1g(i,j,k)

!          write(*,*) mdot(i), pi, dvol, dg(i,j,k), x1b(i), v1g(i,j,k)
...
!          stop
          ...
          if (TkeV.ge.0.1) then
            ...
            v11D_X(i) = v11D_X(i) + dvolx*v1g(i,j,k) 
            mdot_X(i) = mdot_X(i) - 4.*pi*dvolx*x1b(i)*dg(i,j,k)*x1b(i)*v1g(i,j,k)
          endif 
        ...
        v11D_X(i) = v11D_X(i)/(volx+1.e-30)
        mdot_X(i) = mdot_X(i)/(volx+1.e-30)
        write(42,2001) x1b(i), d1D(i), d1D_X(i), e1D(i), e1D_X(i) &
        , v11D(i), v11D_X(i), mdot(i), mdot_X(i)
        if (iincr.eq.5) write(11,*) x1b(i)/3.086e21, mdot_X(i)/2.e33*pi*1.e7 
      enddo
